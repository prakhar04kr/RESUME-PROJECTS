...
const MYSQL_URL = process.env.DATABASE_URL; // e.g. mysql://user:pass@host:3306/dbname
...
// Optional Redis caching removed from this minimal MySQL backend; leaderboard caching can be re-added.
// Mechanism is preserved with direct MySQL query.
...
app.post('/api/auth/login', async (req, res) => {
  ...
  const [users] = await pool.query('SELECT id, email, username, role, password_hash, created_at FROM users WHERE email = ? LIMIT 1', [email]);
  const user = users[0];
  if (!user) {
...
app.get('/api/auth/me', authGuard, async (req, res) => {
  ...
  const [rows] = await pool.query('SELECT id, email, username, role, created_at FROM users WHERE id = ? LIMIT 1', [userId]);
  const user = rows[0];
  if (!user) {
...
app.post('/api/results', authGuard, async (req, res) => {
  ...
  const [rows] = await pool.query('SELECT id, user_id, game_id, wpm, accuracy, time_taken_ms, created_at FROM results WHERE id = ? LIMIT 1', [resultId]);
...
// --------------------
// WebSocket (same protocol as original)
// --------------------
